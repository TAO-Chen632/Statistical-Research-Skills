...
pdf("Part2_fig1.pdf")
ISE$n1 %>% rename("Kernel density" = kernel, "Histogram" = hist,
  ...
  guides(fill = FALSE) +
  labs(
    title = "Integrated squared errors of different estimators\n on samples with size n = 250",
...
pdf("Part2_fig2.pdf")
ISE$n2 %>% rename("Kernel density" = kernel, "Histogram" = hist,
                  "Orthogonal series" = OS) %>%
  pivot_longer(cols = 1:3, names_to = "Estimator", values_to = "ISE") %>%
  ggplot(
    aes(
...
pdf("Part2_fig3.pdf")
ISE$n3 %>% rename("Kernel density" = kernel, "Histogram" = hist,
                  "Orthogonal series" = OS) %>%
  pivot_longer(cols = 1:3, names_to = "Estimator", values_to = "ISE") %>%
  ggplot(
    aes(
...
ISE_mean = data.frame(kernel = c(mean(ISE$n1$kernel), mean(ISE$n2$kernel), mean(ISE$n3$kernel)),
                      OS = c(mean(ISE$n1$OS), mean(ISE$n2$OS), mean(ISE$n3$OS)),
                      hist = c(mean(ISE$n1$hist), mean(ISE$n2$hist), mean(ISE$n3$hist)),
